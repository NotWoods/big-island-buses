...
function applyOffset(input, offset) {
...
const fuseRoutes = new GtfsSearch(['route_long_name']);
const fuseStops = new GtfsSearch(['stop_name']);
...
function requestToParams(request, target) {
...
function loadIndex() {
    return fetch(pathPrefix + 'indexes.json').then((res) => {
        if (!res.ok)
            return undefined;
        return res.json().then((indexData) => {
            const { routes, stops } = indexData;
            return {
                routes: Fuse.parseIndex(routes),
                stops: Fuse.parseIndex(stops),
            };
        });
    });
}
    ...
    switch (message.type) {
        case 'data':
            return loadIndex().then((indexes) => {
                fuseRoutes.initialize(message.routes, indexes === null || indexes === void 0 ? void 0 : indexes.routes);
                fuseStops.initialize(message.stops, indexes === null || indexes === void 0 ? void 0 : indexes.stops);
            });
        case 'search':
            return search$1(message.request);
        default:
