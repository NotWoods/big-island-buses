...
var locatePromise = locateUser(schedulePromise), placeMapMarker, mapLoaded = loadMap();
...
function updateAside(schedule) {
	...
	function generateListItems() {
		for (var item in schedule.routes) {
			...
			listItem.style.borderColor = ["#", schedule.routes[item].route_color].join('');
...
function makeLinksDynamic() {
	var scheduleLinks = document.getElementById("schedule").childNodes;
	var connectLinks = document.querySelectorAll("#connections a");

	for (var i = 0; i < scheduleLinks.length; i++) {
		var stopId = scheduleLinks[i].href.substring(scheduleLinks[i].href.indexOf("stop=") + 5);
		console.log(stopId);
		dynamicLinkNode(Type.STOP, stopId, true, scheduleLinks[i]);
	}

	for (var j = 0; j < connectLinks.length; j++) {
		var routeId = connectLinks[j].href.substring(1, connectLinks[j].href.indexOf("?"));
		dynamicLinkNode(Type.ROUTE, routeId, false, connectLinks[j]);
	}
}

documentPromise.then(function() {uiEvents(); makeLinksDynamic();});
...
function openRoute(route_id) {
	...
	document.getElementById("place-value").textContent = ["Between ", 
		buses.stops[firstStop].stop_name, " - ", buses.stops[lastStop].stop_name].join('');
	document.getElementById("time-value").textContent = [stringTime(earliest), " - ", stringTime(latest)].join('');
	document.getElementById("next-stop-value").textContent = ["Reaches ", buses.stops[closestTripStop].stop_name, " in ", minString].join('');
...
function openTrip(trip_id) {
	...
	for (var i = 0; i < stopSequence.length; i++) {
		...
		for (var j = 0; j < 2; j++) {
			...
			line.className = "line";
			line.style.backgroundColor = ["#", buses.routes[Active.Route.ID].route_color].join('');
...
documentPromise.then(function() {
	if ('serviceworker' in navigator) {
		navigator.serviceworker.register('/sw.js').then(console.log("Service Worker sucess!"));
	}	
})
...
function dragging() {
	...
	function mouseMove(event) {
		mapDiv.style.height = event.screenY;
	}

	document.getElementById("handlebar").addEventListener(function(e) {
		e.preventDefault();
