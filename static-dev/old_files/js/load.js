...
/**
 * Deprecated function: Grabs google_transit.zip and parses the data into a 
 ...
 */
/*function getScheduleDataOld(mode) {
	...
	var fileList = ["agency.txt", "calendar.txt", "fare_attributes.txt", "feed_info.txt", 
	...
	return new Promise(function(resolve, reject) {
		...
		function folderRequest(fileName) {
			return new Promise(function(rqResolve, rqReject) {
				...
				rq.open("GET", ["/google_transit/", fileName].join(''));
		...
		if (mode == "gtfs") {
			rqDone = new Promise(function(rqResolve, rqReject) {
				var rq = new XMLHttpRequest();
				rq.open("GET", "/txt/google_transit.zip");
...
/**
 * Grabs output.json as a GTFSData object for the rest of the program.
 * @return {Promise}     Promise returns GTFS data file for the rest of the program
 */
function getScheduleData() {
	return new Promise(function(resolve, reject) {
		var rq = new XMLHttpRequest();
		rq.open("GET", "/output.json");
...
function locateUser(busPromise, customLocation) {
	return new Promise(function(resolve, reject) {
		...
		if (customLocation) {
			...
			if ("geolocation" in navigator) {navigator.geolocation.getCurrentPosition(locate, function(error) {reject(error);});}
		}
	});
}
...
function pageLink(type, value) {
	...
	switch (type) {
		case Type.ROUTE:
			...
			break;
		case Type.STOP:
			if (Active.Route.ID !== null) {link.push("#!route=", Active.Route.ID);} else {link.push("#!");}
			if (Active.Route.TRIP !== null) {link.push("&trip=", Active.Route.TRIP);}
 ...
 * @param  {Node} node      Element to update, instead of creating a new one.
 * @return {Node}           A element with custom properties
 */
function dynamicLinkNode(type, value, update, nodeToUpdate) {
	var node = nodeToUpdate ? nodeToUpdate : document.createElement("a");
	node.Type = type;
	node.Value = value;
...
function getQueryVariable(variable) {
	var query = "", vars;
	if (window.location.pathname.length > 1 && (variable == "route" || variable == "trip")) {
		var path = window.location.pathname;
		if (path.lastIndexOf("/") > 0) {
			if (variable == "route") return path.substring(1, path.lastIndexOf("/"));
			if (variable == "trip") return path.substring(path.lastIndexOf("/"));
		} else {
			if (variable == "trip") return null;
			if (variable == "route") return path.substring(1);
		}
