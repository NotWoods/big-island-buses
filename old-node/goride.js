...
function parseScheduleSource() {
	...
	readRq.then(function(jsonFiles) {
		...
		busSchedule = variable;
		var jStream = fs.createWriteStream(__dirname + '/output.json');
		if (typeof(callback) == "function") {jStream.on('end', callback());}
		jStream.write(JSON.stringify(variable));
		jStream.end();
	});
}
...
function loadAside(schedule, document, ajaxLinks) {
...
function openRoute(buses, document, route_id) {
...
function openStop(buses, document, ajaxLinks, stop_id) {
...
function openTrip(buses, document, ajaxLinks, trip_id) {
...
function loadDocument(isRoot, ajax, route, trip, stop) {
	...
	return new Promise(function(resolve, reject) {
		htmlPromise.then(function(indexFile) {
			jsdom.env("index.html", function (errors, window) {
  		  ...
  		  if (!ajax) {
  		 	...
  		 	var opend = {r:null,t:null,s:null};
				if (route !== null) {
					opend.r = openRoute(null, document, Active.Route.ID);
					opend.t = openTrip(null, document, ajax, Active.Route.TRIP ? Active.Route.TRIP : opend.r);
				}
				if (stop !== null) {
					opend.s = openStop(null, document, ajax, Active.STOP);
				}
...
app.use("/", function(req, res, next) {
	...
	if (req.query._escaped_fragment_) {
		var fragment = req.query._escaped_fragment_.split("&");
		schedulePromise.then(function() {
			loadDocument(false, true, 
				getFragmentQuery("route", fragment), getFragmentQuery("trip", fragment), getFragmentQuery("stop", fragment)).then(function(doc) {
				res.send(doc);
			}, function(err) {res.status(err).end();});
		});
	} else {
		next();
	}
...
app.get("/", function(req, res, next) {
	schedulePromise.then(function() {
		loadDocument(true, false, null, null, null).then(function(doc) {
			res.send(doc);
		}, function(err) {res.status(err).end();});
	});
});
...
app.get("/:route", function(req, res, next) {
	...
	schedulePromise.then(function() {
		loadDocument(false, false, route, null, stop).then(function(doc) {
			res.send(doc);
		}, function(err) {res.status(err).end();});
	});
});
...
app.get("/:route/:trip", function(req, res, next) {
	var route = req.params.route ? req.params.route : null,
		...
		stop = req.query.stop ? req.query.stop : null;
	schedulePromise.then(function() {
		loadDocument(false, false, route, trip, stop).then(function(doc) {
			res.send(doc);
		}, function(err) {res.status(err).end();});
	});
});
